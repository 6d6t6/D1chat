  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/send") {
      ...
      if (userData.count > 5 || ipData.count > 5) { // More than 5 messages in a minute
        ...
        return new Response(`You are suspended from sending messages for ${Math.max(userData.count, ipData.count)} minute(s). Please check back in ${formatTimeUntil(suspensionTime)}`, { status: 429 });
      }
...
async function getSuspensionTime(ip, env) {
  const ipKey = `ip_${ip}`;
  const ipData = await env.CHAT_KV.get(ipKey, { type: 'json' }) || { suspendedUntil: 0 };
  return ipData.suspendedUntil || 0;
}
